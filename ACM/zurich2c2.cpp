...
bool solve(int D, int P) {
	...
	for (size_t i = 0; i < dh.size(); ++i) {
		...
		bool bpd = false, bnd = false, bed = true;
		bool bpp = false, bnp = false, bep = false;
		for (size_t j = 0; j < dh.size(); ++j) {
			ll v = det(
				dp[dh[j]].x - dp[dh[i]].x,
				dp[dh[j]].y - dp[dh[i]].y,
				dp[dh[ii]].x - dp[dh[i]].x,
				dp[dh[ii]].y - dp[dh[i]].y);
			if (v < 0) bnd = true;
			if (v > 0) bpd = true;
			if (v == 0) bed = true;
		}
		...
		if (!(bpd && bpp) && !(bnd && bnp) && !(bed && bep))
			return true;
	}
	for (size_t i = 0; i < ph.size(); ++i) {
		size_t ii = (i + 1) % ph.size();
		bool bpd = false, bnd = false, bed = false;
		bool bpp = false, bnp = false, bep = true;
		for (size_t j = 0; j < dh.size(); ++j) {
			ll v = det(
				dp[dh[j]].x - pp[ph[i]].x,
				dp[dh[j]].y - pp[ph[i]].y,
				pp[ph[ii]].x - pp[ph[i]].x,
				pp[ph[ii]].y - pp[ph[i]].y);
			if (v < 0) bnd = true;
			if (v > 0) bpd = true;
			if (v == 0) bed = true;
		}
